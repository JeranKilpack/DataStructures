...
//  DataStructures
//
//  Created by Kilpack, Jeran on 4/27/18.
...
public:
    ...
    bool conatains(HashNode<Type> * value);
...
template <class Type>
long Hashtable<Type> :: getNextPrime        //getNextPrime
{
...
template <class Type>
bool Hashtable<Type> :: isPrime(long current)                   //isPrime
{
    if(conidateNumber <= 1)
    {
        return false;
...
long Hashtable<Type> :: handleCollision(long currentPostition)
{
    long shift = 17;
    
    for (long positon = currentPosition + shift; position != currentPosition; position += shift)
    {
...
template <class Type>
void Hashtable<Typ> :: insert(Type value)
{
    this->size++;
